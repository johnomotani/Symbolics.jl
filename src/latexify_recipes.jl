...
function latexify_derivatives(ex)
    return postwalk(ex) do x
        ...
        elseif x.args[1] === :_indefiniteintegral
            var_of_int, integrand = x.args[2:end]
            return Expr(:call, :*,
                        "\\int",
                        var_of_int,
                        integrand
                       )
        elseif x.args[1] === :_definiteintegral
...
function _toexpr(O; latexwrapper = default_latex_wrapper)
    ...
    elseif op isa IndefiniteIntegral
        vars = op.x
        integrand = args[1]
        vars = if vars isa Tuple
            Expr(:call, :(*), _toexpr(vars...))
        else
            _toexpr(vars)
        end
        return Expr(:call, :_indefiniteintegral, vars, _toexpr(integrand))
    elseif op isa DefiniteIntegral
        ...
        return Expr(:call, :_definiteintegral, _toexpr(lower), _toexpr(upper), vars, _toexpr(integrand))
